...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('dryrun.helloWorld', () => {
        vscode.window.showInformationMessage('Hello World from dryrun!!');
    }));
    context.subscriptions.push(vscode.commands.registerCommand('dryrun.helloName', () => {
        vscode.window.showWarningMessage('Hello World from Ascentic!!');
    }));
    context.subscriptions.push(vscode.commands.registerCommand('dryrun.chat', () => {
        panel.webview.postMessage({ message: 'Hi broooooo.....' });
    }));
    context.subscriptions.push(vscode.commands.registerCommand('dryrun.codeman', () => {
        panel = vscode.window.createWebviewPanel('codeman', 'Code Man', vscode.ViewColumn.One, { enableScripts: true,
            // localResourceRoots: [vscode.Uri.joinPath(extensionUri, 'media')] 
        });
        // serve static local content
        // Get path to resource on disk
        const onDiskPath = vscode.Uri.file(path.join(context.extensionPath, 'media', 'logo.png'));
        // And get the special URI to use with the webview
        const logoImgSrc = panel.webview.asWebviewUri(onDiskPath);
